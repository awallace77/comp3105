...
\begin{enumerate}[(a)]
  \item (1\%) Implement a Python function \\
  \texttt{Y, U, obj\_val = kmeans(X, k, max iter=1000)}\\
  Please see \texttt{A3codes.py} for the implementation of \texttt{kmeans(X, k, max iter=1000)}
  \item (1\%) Implement a Python function \\
  \texttt{Y, U, obj\_val = repeatKmeans(X, k, n runs=100)}\\
  Please see \texttt{A3codes.py} for the implementation of \texttt{repeatKmeans(X, k, n runs=100)}
  \item (1\%) Implement a Python function \\
  \texttt{obj\_val, list = chooseK(X, k candidates=[2,3,4,5,6,7,8,9])} \\
  Please see \texttt{A3codes.py} for the implementation of \texttt{chooseK(X, k candidates=[2,3,4,5,6,7,8,9])}
  \begin{table}[H]
    \centering
    \begin{tabular}{|c|c|c|c|c|c|c|c|c|}
      ...
      $k$ & 2 & 3 & 4 & 5 & 6 & 7 & 8 & 9 \\
      \hline
      \texttt{obj\_val} & 2.7363 & 1.6877 & 1.0105 & 0.8407 & 0.7095 & 0.5935 & 0.5062 & 0.4314\\
      \hline
    \end{tabular}
    \caption{Objective values for different $k$}
    \label{tab:q3_objective_values}
  \end{table}
  ...
  \item (2\%) Implement a Python function \\
  \texttt{Y, obj\_val = kernelKmeans(X, kernel\_func, k, init\_Y, max\_iter=1000)} \\
  Please see \texttt{A3codes.py} for the implementation of \texttt{kernelKmeans(X, kernel\_func, k, init\_Y, max\_iter=1000)} \\
\end{enumerate}
\end{document} 
